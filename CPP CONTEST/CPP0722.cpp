...
bint catalan(int n) {
    if (n == 0)
        return "1";
    vti t;
    for (int i = 2 + n; i <= 2 * n - 1; i++) t.push_back(i);
    for (int i = 3; i <= n - 1; i++) {
        int j = i;
        for (int k = 0; k < t.size(); k++) {
            // Tìm UCLN d gi?a j và t[k]
            // Chia c? j và t[k] cho d
            // N?u t[k] == 1 thì lo?i t[k] ra kh?i t
            // N?u j == 1 thì break
            int d = __gcd(j, t[k]);
            j /= d;
            t[k] /= d;
    ...
    bint res = to_string(t[0]);
    for (int i = 1; i < t.size(); i++) res = res * t[i];
    return res;
}
