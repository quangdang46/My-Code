...
void send_all_acceptable_packages(town* source, int source_office_index, town* target, int target_office_index) {
    ...
    int count, i, j, package_weight;

    for(i = 0; i < source_office->packages_count; i++) {
        package_weight = source_office->packages[i].weight;

        if((min_weight <= package_weight) && (package_weight <= max_weight)) {
            count = target_office->packages_count;
            target_office->packages_count++;
            target_office->packages = realloc(target_office->packages, (count + 1) * sizeof(package));
            target_office->packages[count] = source_office->packages[i];

            count = source_office->packages_count;
            ...
            source_office->packages = realloc(source_office->packages, (count - 1) * sizeof(package));
            i--;
        }
    }
}
